...
describe('DocuSign API Tests', () => {
  ...
  test('Should authenticate with DocuSign and get an access token', async () => {
    try {
      ...
      // Get access token
      const tokenUrl = process.env.TOKEN_URL || 'https://account-d.docusign.com/oauth/token';
  ...
  test('Should verify DocuSign Connect webhook endpoint', async () => {
    ...
    try {
      ...
      const connectUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/connect`;
  ...
  // Test if the /instances endpoint exists
  test('Should check if /instances endpoint exists', async () => {
    ...
    try {
      ...
      const instancesUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/instances`;
  ...
  test('Should verify envelope endpoints', async () => {
    ...
    try {
      ...
      const envelopesUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/envelopes?from_date=${fromDateStr}`;
      ...
      if (response.data.envelopes && response.data.envelopes.length > 0) {
        ...
        try {
          const formDataUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/envelopes/${testEnvelope.envelopeId}/form_data`;
        ...
        try {
          const documentsUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/envelopes/${testEnvelope.envelopeId}/documents`;
  ...
  test('Should verify form_data endpoint with example envelope', async () => {
    ...
    try {
      ...
      const formDataUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${specificAccountId}/envelopes/${specificEnvelopeId}/form_data`;
  ...
  test('Should verify form_data endpoint with most recent envelope', async () => {
    ...
    try {
      const formDataUrl = `${process.env.DOCUSIGN_API_BASE}/v2.1/accounts/${accountId}/envelopes/${mostRecentEnvelopeId}/form_data`;
